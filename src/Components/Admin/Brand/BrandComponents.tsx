...
const BrandComponents = () => {
  ...
  const { data: brands, isLoading, refetch } = useGetBrandQuery({});
  const [deleteBrand] = useDeleteBrandMutation({});
  const [putBrand] = usePutBrandMutation();
  ...
  const BraId = (id: string) => {
    setOpen(true);
    const bra = brand?.find((item: Props) => item._id === id);
    if (bra) {
      setNameId(bra._id);
      setName(bra.name);
    }
  };
  const handlePutBrand = async (e: React.FormEvent) => {
    e.preventDefault();
    let valid = true;
